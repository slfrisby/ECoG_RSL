    ...
    for t = 1:length(dataType)
        ...
        for d = 1:length(dimension)
            ...
            for i = 1:length(item)
                ...
                f = plot_frequency_vs_voltage(individualTimecourses.(dataType{t}).all.(dimension{d}).(item{i}).(['patient',sprintf('%02d',patients(q))]),individualTimecourses.voltage.(dimension{d}).(item{i}).(['patient',sprintf('%02d',patients(q))]), 1);
...
for t = 1:length(dataType)
    ...
    for d = 1:length(dimension)
        ...
        for i = 1:length(item)
            ...
            % plot power and phase against voltage
            % reanalysed LH
            f = plot_frequency_vs_voltage(groupTimecourses.(dataType{t}).all.(dimension{d}).(item{i})(reanalysedLH,:),groupTimecourses.voltage.(dimension{d}).(item{i})(reanalysedLH,:), 0.75);
            saveas(f,[figuresDir,'/group/',dataType{t},'_all_',dimension{d},'_',item{i},'_reanalysedLH.png']);
            close(f);
            % "new" LH
            f = plot_frequency_vs_voltage(groupTimecourses.(dataType{t}).all.(dimension{d}).(item{i})(newLH,:),groupTimecourses.voltage.(dimension{d}).(item{i})(newLH,:), 0.75);
            saveas(f,[figuresDir,'/group/',dataType{t},'_all_',dimension{d},'_',item{i},'_newLH.png']);
            close(f);
            % RH
            f = plot_frequency_vs_voltage(groupTimecourses.(dataType{t}).all.(dimension{d}).(item{i})(RH,:),groupTimecourses.voltage.(dimension{d}).(item{i})(RH,:), 0.75);
            ...
            f = plot_frequency_vs_voltage(groupTimecourses.(dataType{t}).all.(dimension{d}).(item{i}),groupTimecourses.voltage.(dimension{d}).(item{i}), 0.75);
            saveas(f,[figuresDir,'/group/',dataType{t},'_all_',dimension{d},'_',item{i},'.png']);
            close(f);

            % plot different frequency ranges
            f = plot_frequency_ranges(groupTimecourses.(dataType{t}).theta.(dimension{d}).(item{i}),groupTimecourses.(dataType{t}).alpha.(dimension{d}).(item{i}),groupTimecourses.(dataType{t}).beta.(dimension{d}).(item{i}),groupTimecourses.(dataType{t}).gamma.(dimension{d}).(item{i}),groupTimecourses.(dataType{t}).highGamma.(dimension{d}).(item{i}), 0.75);
            ...
            for r = 1:length(frequencyRange)
                ...
                f = plot_range_vs_frequency(frequencyRange{r},groupTimecourses.(dataType{t}).(frequencyRange{r}).(dimension{d}).(item{i}),groupTimecourses.(dataType{t}).all.(dimension{d}).(item{i}), 0.75);
