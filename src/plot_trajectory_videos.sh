...
ls "$dirp/figures/trajectories/$items/$dataType/$frequency/full/"*.png | sort > $work/filelist.txt
...
-pix_fmt yuv420p \
-movflags +faststart \
"$dirp/videos/$items/${dataType}_${frequency}.mp4"

done
done

#now do voltage
ls "$dirp/figures/trajectories/$items/voltage/full/"*.png | sort > $work/filelist.txt
# Prepend "file '", append "'" to each line, and make each frame 0.1 seconds in duration 
sed -i "s/^/file '/; s/$/'\nduration 0.1/" "$work/filelist.txt"
