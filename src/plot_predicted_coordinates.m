    ...
    if strcmp(dataType{t},'voltage')
        ...
        for w = 1:length(waveletCentre)
            ...
            if w == 1
                ...
                ylim([-0.2,0.2])
                ylabel('Dimension 1', 'FontSize', 12, 'Units', 'normalized', 'Position', [-0.0485, 0.5, 0]) % this is the y-axis positioning that plots created with plot_three_dimensions get naturally. Don't worry about positioning when running on the login node - it looks fine on the submit node!
                box on
                ...
                if ~exist([root,'/results/figures/trajectories/allitems/',dataType{t},'/full/'])
                    mkdir([root,'/results/figures/trajectories/allitems/',dataType{t},'/full/'])
                end
                print(fig,[root,'/results/figures/trajectories/allitems/',dataType{t},'/full/',sprintf('%04d',waveletCentre(w)),'.png'],'-dpng','-r600');
                ...
                % save
                if ~exist([root,'/results/figures/trajectories/allitems/',dataType{t},'/panel/D2/'])
                    mkdir([root,'/results/figures/trajectories/allitems/',dataType{t},'/panel/D2/'])
                end
                print(panelD2,[root,'/results/figures/trajectories/allitems/',dataType{t},'/panel/D2/',sprintf('%04d',waveletCentre(w)),'.png'],'-dpng','-r600');
                % close
                close(panelD2)
                ...
                % save
                if ~exist([root,'/results/figures/trajectories/allitems/',dataType{t},'/panel/D3/'])
                    mkdir([root,'/results/figures/trajectories/allitems/',dataType{t},'/panel/D3/'])
                end
                print(panelD3,[root,'/results/figures/trajectories/allitems/',dataType{t},'/panel/D3/',sprintf('%04d',waveletCentre(w)),'.png'],'-dpng','-r600');
                % close
                close(panelD3)
        ...
        for f = 1:length(frequencyRange)
            ...
            for w = 1:length(waveletCentre)
                ...
                if w == 1
                    ...
                    % save the full figure
                    if ~exist([root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/full/'])
                        mkdir([root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/full/'])
                    end
                    print(fig,[root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/full/',sprintf('%04d',waveletCentre(w)),'.png'],'-dpng','-r600');
                    ...
                    set(panelD2,'PaperPositionMode','auto')
                    % save
                    if ~exist([root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/panel/D2/'])
                        mkdir([root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/panel/D2/'])
                    end
                    print(panelD2,[root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/panel/D2/',sprintf('%04d',waveletCentre(w)),'.png'],'-dpng','-r600');
                    % close
                    close(panelD2)
                    ...
                    set(panelD3,'PaperPositionMode','auto')
                    % save
                    if ~exist([root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/panel/D3/'])
                        mkdir([root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/panel/D3/'])
                    end
                    print(panelD3,[root,'/results/figures/trajectories/allitems/',dataType{t},'/',frequencyRange{f},'/panel/D3/',sprintf('%04d',waveletCentre(w)),'.png'],'-dpng','-r600');
                    % close
                    close(panelD3)
